...
def index(request):
    ...
    response = urllib2.urlopen('http://184.106.153.149/channels/113321/fields/1.json?results=1')
    ...
    if query_set is not None:
        if query_set.entry_id < entry_id:
            ...
            query_set.save()
            if light_value > query_set.alert_value:
                urllib2.urlopen(sms_get)

    else:
        sensor_data = SensorData(entry_id=0, light_value=0, create_time=datetime.now(), version=0)
        sensor_data.save()
